...
def mxgemm_kernel(a_ptr, b_ptr, output_ptr,  #
    ...
    Features:
    - Descriptor loads (tl.make_tensor_descriptor) for A, B matrices and scales
    - Pre-shuffled scale layout for coalesced memory access
...
def run_mxfp_gemm(
    ...
    Enhanced with automatic tuning for num_stages (software pipelining depth).

    Args:
        M, N, K: Matrix dimensions
        BLOCK_M, BLOCK_N, BLOCK_K: Tile sizes (use smaller BLOCK_K for large K dimensions)
...
@pytest.mark.parametrize("M, N, K", [(2048, 2048, 1024)])
@pytest.mark.parametrize("BM, BN, BK", [(128, 128, 128), (256, 256, 128)])
@pytest.mark.parametrize("dtype_a, dtype_b", [('float8_e5m2', 'float8_e5m2'), ('float8_e4m3', 'float8_e4m3')])
@pytest.mark.parametrize("num_stages", [2, 3])
@pytest.mark.skipif(not is_hip_gfx1250(), reason="Scaled dot with TDM is only tested on gfx1250.")
def test_mxgemm(M, N, K, BM, BN, BK, dtype_a, dtype_b, num_stages):
    """Test MXFP GEMM with multi-stage pipelining for large problem sizes."""
...
def main():
    parser = argparse.ArgumentParser(description='MXFP GEMM Kernel with Software Pipelining')
    ...
    print(f"  Num stages: {args.num_stages or 'auto'}")
